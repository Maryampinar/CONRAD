...
public class DefectPixelInterpolation {
	...
	public Grid2D interpolateSpectral(Grid2D image, Grid2D mask, int maxIter, boolean zeroPadding)
	{
		...
		for(int i = 0; i < maxIter; i++)
		{
			...
			if(i > 0)
			{
				maxDeltaE_G_Ratio = Math.abs(maxDeltaE_G - lastEredVal/maxDeltaE_G);  		// check the conversion
			}
			
			lastEredVal = maxDeltaE_G;														// store it
			
			//Compute the corresponding linepair s2, t2:
			int s2 = (s1 > 0) ? dim[0] - (s1 % dim[0]) : s1; 								// mirror it, if its larger set zero
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/ciptmp/we08qame/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/testimg.bmp";
		String filenameMask = "/proj/ciptmp/we08qame/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/mask.bmp";
		...
		defectImage.getGridOperator().multiplyBy(defectImage, mask);  					// multiply ideal image * mask
